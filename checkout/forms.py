...
class CCExpField(forms.MultiValueField):
    ...
    def clean(self, value):
        ...
        if date.today() > exp:
            raise forms.ValidationError("The expiration date you entered \
                                            is in the past.")
        return exp
...
class SalePaymentForm(forms.Form):
    number = CreditCardField(required=True,
                                label="Card Number",
                                widget=forms.TextInput(attrs={'id': 'creditcard-number'}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.total_sale_price = kwargs.pop('total_sale_price', None)
